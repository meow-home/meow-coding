...
| A 1px line is visible right beside the Windows caption buttons even when the overlay color matches the bar | The OS paints its caption strip ~0.5px narrower than the `11.5rem` the renderer reserves (`.title-bar-win32 .title-bar-right { margin-right }`), so a sliver of the page shows through. `.title-bar` therefore paints `var(--bg)` — the strip's own surface — instead of leaving `body`'s radial gradient exposed (`window-chrome.test.ts` guards this); it also fixes the same seam in the FileViewer/GitViewer popups |
