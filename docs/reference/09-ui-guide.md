...
| `ToolCallCard.tsx` | One tool call in a compact micro-bar layout (badge, file/command summary, status hairline, expandable diff/output body). Memoized. |
| `ToolCluster.tsx` | A run of consecutive tool calls wrapped in a single collapsible group — `Ran N commands` header with status counts (ok / running / failed pills), inner `.tool-call` cards stacked with a 1px hairline between them. Inner rows keep their own expand toggle, so the user can still drill into one tool without opening the whole group. Clustering happens at render time inside `ChatPanel.tsx`; a lone tool call renders as a bare `.tool-call` (no wrapper). |
