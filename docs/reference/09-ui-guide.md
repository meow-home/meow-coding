...
| `ToolCluster.tsx` | A run of consecutive tool calls wrapped in a single collapsible group — `Ran N commands` header with status counts (ok / running / failed pills), inner `.tool-call` cards stacked with a 1px hairline between them. Always passes `collapseWhileRunning` to every inner `ToolCallCard` so clustered steps stay collapsed even while running (the cluster header already shows progress). Inner rows keep their own expand toggle, so the user can still drill into one tool by clicking it. Clustering happens at render time inside `ChatPanel.tsx`; a lone tool call renders as a bare `.tool-call` (no wrapper). |
...
- **Tool-call clustering.** Consecutive `tool` items fold into a single `.tool-cluster` group at
  render time (see `ToolCluster.tsx`). Reasoning (`<details class="chat-reasoning">`) and assistant
  text break the run, so they stay as their own bubbles. `content-visibility: auto` is preserved on
