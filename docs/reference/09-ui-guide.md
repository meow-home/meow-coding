...
| `PaneHeader.tsx` | Status dot (which carries the status as its accessible name — `role="img"` + the status label, with any exit code folded in), the session name, the menu (inject / log / stop / restart / background / Files / Processes / delete — inject/log/stop/restart exist only on the parked PTY path; a native session's lifecycle lives in its sidebar row; Files opens the Files overlay, Processes opens the Processes overlay), and the todo pill (`TodoPill`) rendered inline in `.pane-actions`, immediately left of the action (⋮) button, when a todo list is present. The `...` button is the shared `.icon-btn`. No status word (the dot already shows it) and no git readout (the status bar owns branch + dirty count) |
...
| Component | Responsibility |
|---|---|
| `ChatPanel.tsx` | The container: subscribes to chat events, owns feed state (items / todos / queue / pendingPrompt), rAF-batches stream deltas, and merges transcript loads with live rows by stable id. Tool start/result events upsert the same row, and a result recreates a missing start row, so an in-flight transcript-load race cannot hide tool output. At render time the feed map folds **every** `tool` item into a `cluster` row via `clusterToolRows` (a pure helper that walks `items`) — so the agent's back-to-back tool bursts read as one tight group, and even a lone tool call renders as a 1-tool cluster with the same chrome. The helper also drops empty assistant messages (no text / reasoning / images) entirely from the feed; the agent's loop persists these between tool batches, so without that drop otherwise-consecutive tools would scatter across 1-tool clusters on reload. State stays flat, so paging and live upserts are unchanged. Renders feed (`FeedMessage` with hover copy action bar for assistant messages) + composer + context footer. The todo list is a compact pill (`TodoPill`, direction 1 "mission badge") seated in the pane header next to the action (⋮) button — not a lane surface — with an orange `TODO` tag, progress ring + count, a pulsing status dot, and a scroll-capped dropdown; its state is lifted up to `Pane` via `onTodosChange` so the header pill (a sibling) can render it. An incoming delegated message renders as a user bubble with a `delegation` badge and a `From session: <name>` label (`.chat-delegation-label`); the metadata travels on `FeedItem`/`toFeedItem` so a reload reproduces it, and `direction: 'result'` rows stay unlabelled. The permission/question prompt with type badge tag is rendered in-flow at the top of the chat input card (never overlays the chat history). The composer's `chat-footer` row — below the input row, still inside the composer — puts the mode on the left (`chat-footer-context`) and the model/variant selectors plus the context readout on the right (`chat-footer-controls`). Memoized. |
...
| `useChatScroll.ts` | Feed scroll controller: follow / anchored / manual modes, turn-top anchoring, jump-to-end button. Detach is gesture-driven: only wheel, touch, keyboard nav, or a scrollbar drag leaves follow mode — a `scroll` event observed off the bottom while the DOM itself grows (streamed deltas, `content-visibility` rows resolving their height, history paging) must NOT detach it, so `scrollEventAction()` classifies each scroll event into none/follow/detach instead of trusting the position alone. After the user scrolls away (manual mode) following only resumes at the literal bottom — re-arming from the loose 80px follow zone would make a slight upward nudge on a long, still-streaming turn snap the viewport back down next delta (up/down jitter). Pure geometry helpers live in `chat-scroll-geometry.ts`. |
| `ToolCallCard.tsx` | One tool call in a compact micro-bar layout (badge, file/command summary, status hairline, expandable diff/output body). Memoized. When `collapseWhileRunning` is true (set by `ToolCluster`), the card never auto-opens — even while the call is `pending` — so a long run of tool steps stays collapsed. |
| `ToolCluster.tsx` | **Every** tool call renders inside a cluster, even lone ones (a 1-tool cluster still gets the `Ran 1 command` chrome). A run of consecutive tool calls wraps in a single collapsible group — `Ran N commands` header with status counts (ok / running / failed pills), inner `.tool-call` cards stacked with a 1px hairline between them. Always passes `collapseWhileRunning` to every inner `ToolCallCard` so clustered steps stay collapsed even while running (the cluster header already shows progress). Inner rows keep their own expand toggle, so the user can still drill into one tool by clicking it. Clustering happens at render time inside `ChatPanel.tsx`. |
...
- **Tool-call clustering.** Every `tool` item renders inside a `.tool-cluster` group at render time
  ...
  consecutive tools across 1-tool clusters on reload. `FeedMessage` already drops them at render,
  so the visible result matches a fresh live session. `content-visibility: auto` is preserved on
  each inner `.tool-call` — clustering changes layout only, not the perf budget.
- **Steps inside a cluster stay collapsed by default** — `ToolCluster` passes `collapseWhileRunning`
  to every inner `ToolCallCard`, so even a `pending` step renders closed. The cluster header
  (`Ran N commands` + status pills) already shows progress; a long run of steps that each pop open
  would scatter the feed and defeat the cluster. The user can still click any step to peek.
  A `pending → done` transition re-keys the card so it closes even if the user had clicked it open
  during the run.
