...
- `session-file-store.ts` (in `agent/`) — per-session JSONL storage; `MainApp.sessionFiles` is the shared instance, and the app-ready chain calls its `migrateLegacy()` after the v0.37 reset.
