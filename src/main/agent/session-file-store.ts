...
export class SessionFileStore {
   ...
   * Kept off the constructor: `MainApp` builds the manager (and therefore this
   * store) as a field initializer at module load, and the v0.37 reset in
   * `index.ts` deletes `sessions.json` before any workspace is activated.
   * Migrating at construction would run *before* that delete and resurrect the
   * file it just backed up. Best-effort: a failure must not break the store.
   */
  migrateLegacy(): void {
    try { migrateSessions(this.rootDir) } catch { /* best effort */ }
  }
