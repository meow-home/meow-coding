...
| `manager.ts` | `ExternalApiManager`: copies the CLI to `userData/bin/` on start (a copy failure is logged and reported as `error`, the server still starts), starts/stops the server with the setting (`stop()` clears `port`, keeps `enabled`), status for the Settings tab, `installClaudeSkill()`. |
...
on each connection retry and once on a 401.
