...
| `PaneHeader.tsx` | Pane title bar: status dot, git info, menu (inject/log/stop/restart/Files/Open Folder/Open in VS Code/delete — the first four only on the parked PTY path; Files opens the Files overlay); shows a confirm dialog before deleting a session. |
...
| `file-content/FileContentView.tsx` | Toolbar + body of a file (Shiki-highlighted code, rendered markdown or plain `<pre>`, interactive code editor textarea, Save button with `● Unsaved` state, `Ctrl+S`/`Cmd+S` keyboard shortcut, Raw/Preview toggle, Copy, Open in VS Code), shared by the popup `FileViewer` window and the overlay tab. |
