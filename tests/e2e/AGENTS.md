...
| `composer.spec.ts` | The composer card's one row: at rest the card is one text line tall, with the button inside the card at its right edge and centred on that line; the field grows with its content, caps at 8 lines (then scrolls) and shrinks back when emptied; Send is disabled while the field is empty (whitespace is not content), enabled with text, and clears the field when clicked; Enter sends and Shift+Enter inserts a newline; while a turn runs the slot is Stop with a `Processing...` placeholder and Enter still queues; editing a queued message swaps Stop for `Save edit`; Stop ends the turn and brings Send back. Copies `sidebar-sessions.spec.ts`'s gated mock LLM, which holds the response body open so a turn is genuinely in flight rather than raced. |
