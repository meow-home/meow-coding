...
describe('SessionFileStore legacy migration', () => {
  let root: string
  beforeEach(() => { root = mkdtempSync(path.join(tmpdir(), 'meow-fsmig-')) })
  afterEach(() => rmSync(root, { recursive: true, force: true }))

  it('migrates a legacy sessions.json on migrateLegacy', () => {
  ...
  it('does not migrate at construction, so a pending reset can still delete sessions.json', () => {
    writeFileSync(path.join(root, 'sessions.json'), JSON.stringify([]))
    const store = new SessionFileStore(root)
    expect(existsSync(path.join(root, 'sessions.json'))).toBe(true)
    expect(existsSync(path.join(root, '.sessions-migrated-v1'))).toBe(false)
    expect(store.list()).toEqual([])
  })
})
